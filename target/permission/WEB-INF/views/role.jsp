...
<input id="gritter-light" checked="" type="checkbox" class="ace ace-switch ace-switch-5" title=""/>
        ...
        <div class="tabbable" id="roleTab">
            ...
            <div class="tab-content">
                ...
                <div id="roleUserTab" class="tab-pane fade">
                    ...
                    <select multiple="multiple" size="10" name="roleUserList" id="roleUserList" title="">
                    </select>
...
<script>
    $(function () {
        var roleMap = {};           // 存储map格式角色信息
        var lastRoleId = -1;
        var selectFirstTab = true;  // true-角色与权限关系维护，false-角色与用户关系维护
        var hasMultiSelect = false; // 控制 Bootstrap Dual Listbox 只触发一次，之后 refresh 即可
        ...
        function handleRoleSelected(roleId) {
            ...
            $('#roleTab').find('a:first').trigger('click');
            // 角色与权限关系维护，加载角色与权限列表
            if (selectFirstTab) {
                loadRoleAcl(roleId);
            }
        ...
        $(".saveRoleAcl").click(function (e) {
            e.preventDefault();
            if (lastRoleId === -1) {
                showMessage("保存角色与权限点的关系", "请先左侧选择需要操作的角色", false);
                return;
            }
            $.ajax({
        ...
        function getTreeSelectedId() {
            ...
            return v.length > 0 ? v.substring(1): v; // 去掉第一个逗号
        }

        // tab点击事件
        $("#roleTab").find("a[data-toggle='tab']").on("shown.bs.tab", function(e) {
            if(lastRoleId === -1) {
